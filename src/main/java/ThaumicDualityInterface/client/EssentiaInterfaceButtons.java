...
public class EssentiaInterfaceButtons implements ICustomButtonDataObject {
    ...
    public EssentiaInterfaceButtons(final boolean fullBlock) {
    ...
    public void initCustomButtons(int guiLeft, int guiTop, int xSize, int ySize, int xOffset, int yOffset,
        ...
        this.switcher = new GuiTabButton(
            ...
            !fullBlock ? ItemAndBlockHolder.PART_ESSENTIA_INTERFACE.stack()
                : ItemAndBlockHolder.BLOCK_ESSENTIA_BLOCK.stack(),
    ...
    public boolean actionPerformedCustomButtons(final GuiButton btn) {
        if (btn == this.switcher) {
            ThaumicDualityInterface.proxy.netHandler.sendToServer(new CPacketSwitchEssentiaGuis());
            return true;
